...
class ScenesClassification(Dataset):
...
class DenseNet(nn.Module):
    def __init__(self, num_blocks, growth_rate, num_classes):
        ...
        self.conv1 = nn.Conv2d(
            3, 2*growth_rate, kernel_size=7, stride=2, padding=3, bias=False)
...
if __name__ == '__main__':
    ...
    root_path = '/scenes_classification'
    ...
    train_dataset = ScenesClassification(X_train, y_train, transform=transform)
    val_dataset = ScenesClassification(X_val, y_val, transform=transform)
    test_dataset = ScenesClassification(X_test, y_test, transform=transform)
    ...
    model = DenseNet(num_blocks=[6, 12, 24, 16],
                     growth_rate=32, num_classes=n_classes)
